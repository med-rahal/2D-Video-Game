...
//structure contains computer coordinates//
...
typedef struct{
 ...
 //structure Contains the images needed to render the pick screen//
typedef struct {
   SDL_Surface * backPicker; //The background//
   SDL_Rect backPos; //The position of background//
   SDL_Surface * backbut; //image of back button//
   SDL_Surface * backbut2; //2 nd image of back button(MouseMotion)//
   SDL_Rect  backbutPos; // back button position//
   char pick; // Choice of the user chose it can be X or O//
}picker;
 

 //struct contains one single line and its position for indicating the win state//
 struct lines{
  SDL_Surface * line; // Line image//
  SDL_Rect linePos; //Line image position//
};

 // Contains all the data of the game play screen//
typedef struct{
  ...
  SDL_Rect scorePos;  // score postion on the screen //
